...
func (h *HandlerMQTT) ProvideWorkpiece(client mqtt.Client, msg mqtt.Message) {
	...
	if err := deliveryMQTT.ResponseOk(h.log, client, reqTopic, respTopic, 1, false, "storage has provided workpiece successfully", nil); err != nil {
		deliveryMQTT.ResponseError(h.log, client, reqTopic, respTopic, 1, false, err)
		return
	}
}
...
func (h *HandlerMQTT) AcceptWorkpiece(client mqtt.Client, msg mqtt.Message) {
	...
	if err := deliveryMQTT.ResponseOk(h.log, client, reqTopic, respTopic, 1, false, "storage has accepted workpiece successfully", nil); err != nil {
		deliveryMQTT.ResponseError(h.log, client, reqTopic, respTopic, 1, false, err)
		return
	}
}
...
func (h *HandlerMQTT) PushMetrics(ctx context.Context, client mqtt.Client) {
	...
	if err := deliveryMQTT.ResponseOk(h.log, client, *new(string), pushMetrics, 1, true, "got storage metrics sucessfully", metrics); err != nil {
		deliveryMQTT.ResponseError(h.log, client, *new(string), pushMetrics, 1, false, err)
		return
	}
}
